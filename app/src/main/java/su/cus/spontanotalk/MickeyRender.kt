...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    animFrame *= .25;
    house(uv-vec2(33.-4.*fract(animFrame),.2),vec2(.4+.1*sin(floor(animFrame)),.5+.3*sin(floor(animFrame))),-.2);
    animFrame *= 1.4;
    ...
    vec3 mLegLL = vec3(.0,.07, 45.*clamp(sin(animTime-73./8.*PI),-.88,1.));
    vec3 mShoeUL = vec3(.0,.0, -45.*clamp(sin(animTime-.98*PI),-99.,.0));
